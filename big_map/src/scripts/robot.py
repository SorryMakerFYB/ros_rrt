...
class robot:
    ...
    def __init__(self, name):
        ...
        self.map_size=rospy.get_param('~map_size',10000)
        self.map_name=name+'/map'
        self.map=OccupancyGrid()
        self.map.data=[-1]*self.map_size*self.map_size
        self.map_publisher=rospy.Publisher(self.map_name,OccupancyGrid,queue_size=10)
        #定义探测半径
        self.radius=100
...
#创建并更性地图
    def mapUpdate(self):
        ...
        for i in range(pos_xy[0]-100,pos_xy[0]+100):
            for j in range(pos_xy[1]-100,pos_xy[1]+100):
                if (norm(pos_xy[0]-i,pos_xy[1]-j)<self.radius):
                    tmp=[i,j]
                    freeSpace.add(point_of_index(self.map,tmp))
        for i in freeSpace:
            self.map.data[i]=0
